...
\begin{equation}
{}^i V_i = \begin{bmatrix} \omega_i \\ v_{i,x}^b \\ v_{i,y}^b \end{bmatrix} \in \mathfrak{se}(2)
\end{equation}

\noindent Following Modern Robotics (Lynch \& Park) convention: angular velocity first, then linear velocities.

\noindent \textbf{Spatial twist} (velocity in world frame):
\begin{equation}
{}^s V_i = \begin{bmatrix} \omega_i \\ \dot{x}_i \\ \dot{y}_i \end{bmatrix} = \text{Ad}_{T_{si}} \cdot {}^i V_i
\end{equation}
...
\begin{equation}
{}^i F_i = \begin{bmatrix} \tau_i \\ f_{i,x}^b \\ f_{i,y}^b \end{bmatrix} \in \mathfrak{se}(2)^*
\end{equation}

\noindent measured in the body frame. Following Modern Robotics convention: torque first, then forces along local x, y axes.
...
\begin{equation}
B_i = \begin{bmatrix} s_{\omega,i} \\ s_{x,i} \\ s_{y,i} \end{bmatrix} \in \mathfrak{se}(2)
\end{equation}

\noindent Following Modern Robotics convention: angular component first, then linear components.

For a revolute joint at position $\mathbf{r} = [r_x, r_y]^T$ from the end-effector:
\begin{equation}
B_i^{\text{rev}} = \begin{bmatrix} 1 \\ r_y \\ -r_x \end{bmatrix}
\end{equation}

For a prismatic joint along direction $\hat{v} = [v_x, v_y]^T$:
\begin{equation}
B_i^{\text{pri}} = \begin{bmatrix} 0 \\ v_x \\ v_y \end{bmatrix}, \quad \|\hat{v}\| = 1
\end{equation}
...
\begin{equation}
D_d = \text{diag}(d_\theta, d_x, d_y), \quad K_d = \text{diag}(k_\theta, k_x, k_y)
\end{equation}

\noindent Following Modern Robotics convention: angular parameter first, then linear parameters. This allows independent tuning of compliance along each degree of freedom in the body frame.
...
\begin{equation}
a_t = [d_\theta^L, d_x^L, d_y^L, k_\theta^L, k_x^L, k_y^L, d_\theta^R, d_x^R, d_y^R, k_\theta^R, k_x^R, k_y^R]
\label{eq:action_se2}
\end{equation}

\noindent Following Modern Robotics convention: angular parameters first, then linear parameters.

Actions are normalized to $[-1, 1]$ and scaled to bounds:
\begin{align}
D_d &\in [D_{\min}, D_{\max}] = [0.5, 20] \text{ N·m·s/rad (angular)}, [1, 50] \text{ N·s/m (linear)} \\
K_d &\in [K_{\min}, K_{\max}] = [5, 100] \text{ N·m/rad (angular)}, [10, 200] \text{ N/m (linear)}
\end{align}
