...
We use the coordinate representation:
\begin{equation}
\xi = \begin{bmatrix} v_x \\ v_y \\ \omega \end{bmatrix} \in \mathbb{R}^3
\end{equation}
...
The logarithm map $\log: \text{SE}(2) \to \mathfrak{se}(2)$ is:
\begin{equation}
\log(T) = \begin{bmatrix} V^{-1}(\omega) p \\ \omega \end{bmatrix}
\end{equation}
...
For $T = (R, p)$:
\begin{equation}
\text{Ad}_T = \begin{bmatrix} R & \hat{p} R \\ 0 & 1 \end{bmatrix} \in \mathbb{R}^{3 \times 3}
\end{equation}

where $\hat{p}$ is the skew-symmetric matrix of $p$.

In coordinate form:
\begin{equation}
\text{Ad}_T \cdot \xi = \begin{bmatrix} R v + \hat{p} R \omega \\ \omega \end{bmatrix}
\end{equation}
...
- Body frame velocity: ${}^i V = [v_x^b, v_y^b, \omega]^T$

Compute spatial velocity:
\begin{equation}
{}^s V = \begin{bmatrix} R \cdot [v_x^b, v_y^b]^T \\ \omega \end{bmatrix} = \begin{bmatrix} \dot{x} \\ \dot{y} \\ \omega \end{bmatrix}
\end{equation}

\textbf{Implementation}:
\begin{lstlisting}[language=Python]
# src/se2_math.py
def body_to_world_velocity(pose, body_velocity):
    x, y, theta = pose
    v_b = body_velocity[:2]
    ...
    return np.array([v_s[0], v_s[1], omega])
\end{lstlisting}

\paragraph{Spatial to Body}
Given:
- Current pose: $T_{si} = (R, p)$
- Spatial velocity: ${}^s V = [\dot{x}, \dot{y}, \omega]^T$

Compute body velocity:
\begin{equation}
{}^i V = \begin{bmatrix} R^T \cdot [\dot{x}, \dot{y}]^T \\ \omega \end{bmatrix} = \begin{bmatrix} v_x^b \\ v_y^b \\ \omega \end{bmatrix}
\end{equation}

\textbf{Implementation}:
\begin{lstlisting}[language=Python]
# src/se2_math.py
def world_to_body_velocity(pose, world_velocity):
    x, y, theta = pose
    v_w = world_velocity[:2]
    omega = world_velocity[2]

    R = rotation_matrix(theta)
    v_b = R.T @ v_w

    return np.array([v_b[0], v_b[1], omega])
\end{lstlisting}
...
- Current pose: $T_{si} = (R, p)$
- Spatial velocity: ${}^s V = [\dot{x}, \dot{y}, \omega]^T$
- Spatial acceleration: ${}^s \dot{V} = [\ddot{x}, \ddot{y}, \dot{\omega}]^T$

Compute body acceleration with Coriolis correction:
\begin{equation}
{}^i \dot{V} = \begin{bmatrix} R^T \cdot [\ddot{x}, \ddot{y}]^T - \omega \times (R^T \cdot [\dot{x}, \dot{y}]^T) \\ \dot{\omega} \end{bmatrix}
\end{equation}
...
For a planar 2-DOF arm (translational joints in x, y):
\begin{equation}
M = \begin{bmatrix} m & 0 \\ 0 & m \end{bmatrix}, \quad J_b = \begin{bmatrix} \cos\theta & 0 \\ \sin\theta & 0 \\ 0 & 1 \end{bmatrix}
\end{equation}

This simplifies to:
\begin{equation}
\Lambda_b = \text{diag}(m, m, I_z)
\end{equation}

where $I_z$ is the rotational inertia.

\textbf{Implementation}:
\begin{lstlisting}[language=Python]
# src/se2_dynamics.py
def compute_task_space_inertia(self, current_pose):
    # For planar translational arms
    m = self.params.mass
    I = self.params.inertia
    Lambda_b = np.diag([m, m, I])
    return Lambda_b
\end{lstlisting}
...
In SE(2), a unit screw has the form:
\begin{equation}
\mathcal{S} = \begin{bmatrix} \omega \\ v \end{bmatrix} \in \mathfrak{se}(2), \quad \|\mathcal{S}\| = 1
\end{equation}

\paragraph{Revolute Joint}
For a revolute joint at position $p_j = [x_j, y_j]^T$ in the end-effector body frame:
\begin{equation}
\mathcal{S}^{\text{rev}} = \begin{bmatrix} 1 \\ -y_j \\ x_j \end{bmatrix} \quad \text{(normalized)}
\end{equation}

The linear velocity component is perpendicular to the position vector, with magnitude equal to the distance from the joint.
...
No angular component, pure translation along the joint direction.
